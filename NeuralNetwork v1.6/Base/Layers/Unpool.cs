...
namespace NeuralNetwork.Base.Layers
{
    public class Unpool : ILayer
    {
        ...
        public Unpool(ILayer lastLayer)
        {
            ...
            output = new Tensor4(2 * width - 1, 2 * height - 1, deep, bs);
        }
                ...
                for (int z = 0; z < output.deep; z++)
                    for (int y = 0; y < output.height; y += 2)
                        for (int x = 0; x < output.width; x += 2)
                            output[d, z, y, x] = input[d, z, y / 2, x / 2];
        ...
        public Tensor4 Backward()
        {
            ...
            for (int d = 0; d < output.bs; d++)
                for (int z = 0; z < output.deep; z++)
                    for (int y = 0; y < output.height; y += 2)
                        for (int x = 0; x < output.width; x += 2)
                            lastDelts[d, z, y / 2, x / 2] = delts[d, z, y, x];
            return lastDelts;
        }
